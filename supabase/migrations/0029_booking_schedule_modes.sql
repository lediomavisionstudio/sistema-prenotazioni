...
-- Migration additiva: non modifica prenotazioni, turni, waitlist o RPC.
...
alter table venues
  ...
  add column if not exists booking_slot_interval_minutes int not null default 30;
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'venues_booking_slot_interval_minutes_check'
  ) then
    alter table venues
      add constraint venues_booking_slot_interval_minutes_check
      check (booking_slot_interval_minutes in (15, 30, 45, 60));
  end if;
end $$;
...
comment on column venues.booking_slot_interval_minutes is
  'Intervallo in minuti usato per generare gli slot in modalita orari liberi.';
